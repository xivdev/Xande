...
public class MdlFileBuilder {
    ...
    private List< byte > GetVertexData( MeshPrimitive primitive, int index, Vertex.VertexType type, Vertex.VertexUsage usage, Dictionary< int, int >? blendIndexDict = null ) {
        ...
        switch( usage ) {
            case Vertex.VertexUsage.Position:
                ...
                if( positions != null && positions.Count > index ) { vector4 = new Vector4( positions[ index ], 1 ); }

                break;
            case Vertex.VertexUsage.BlendWeights:
                var blendWeights = primitive.GetVertexAccessor( "WEIGHTS_0" )?.AsVector4Array();
                if( blendWeights != null && blendWeights.Count > index ) { vector4 = blendWeights[ index ]; }
            ...
            case Vertex.VertexUsage.BlendIndices:
                ...
                if( blendIndices != null && blendIndices.Count > index ) {
                    ...
                    if( blendIndexDict != null ) {
                        for( var i = 0; i < 4; i++ ) {
                            if( blendIndexDict.ContainsKey( ( int )blendIndices[ index ][ i ] ) ) { vector4[ i ] = blendIndexDict[ ( int )blendIndices[ index ][ i ] ]; }
                        }
                    }
                }
            ...
            case Vertex.VertexUsage.Normal:
                ...
                if( normals != null && normals.Count > index ) { vector4 = new Vector4( normals[ index ], 0 ); }
            ...
            if( positions != null ) {
                for( var vertexId = 0; vertexId < positions.Count; vertexId++ ) {
                    ...
                    for( var decId = 0; decId < vertexDeclarations[ 0 ].VertexElements.Length; decId++ ) {
                        ...
                        if( !streams.ContainsKey( ve.Stream ) ) { streams.Add( ve.Stream, new List< byte >() ); }
    ...
    public int CompareShapeValueStructs( MdlStructs.ShapeValueStruct a, MdlStructs.ShapeValueStruct b ) {
        ...
        if( replacingCompare == 0 ) { return a.BaseIndicesIndex.CompareTo( b.BaseIndicesIndex ); } else { return replacingCompare; }
    }
        ...
        try {
            ...
            if( skeleton != null ) {
                for( var id = 0; id < skeleton.JointsCount; id++ ) {
                    ...
                    if( !String.IsNullOrEmpty( boneString ) ) { allBones.Add( boneString ); }
                }
            }
            ...
            foreach( var meshIndex in _meshes.Keys ) {
                foreach( var submeshIndex in _meshes[ meshIndex ].Keys ) {
                    ...
                    foreach( var primitive in mesh.Primitives ) {
                        var positions = primitive.GetVertexAccessor( "POSITION" )?.AsVector3Array();
                        if( positions != null ) { vertexCount += positions.Count; }
                    }
                }
            }
            ...
            foreach( var meshIndex in _meshes.Keys ) {
                ...
                foreach( var submeshIndex in _meshes[ meshIndex ].Keys ) {
                    ...
                    try {
                        ...
                        } else { PluginLog.Debug( "Mesh contained no extras." ); }
                    } catch( Exception ex ) { PluginLog.Error( $"Error parsing mesh extras: {ex}" ); }
                    ...
                    foreach( var primitive in mesh.Primitives ) {
                            ...
                            for( var i = 0; i < primitive.MorphTargetsCount; i++ ) {
                                ...
                                if( shapePositions != null && shapeNormals != null && positions != null && normals != null ) {
                                    for( var j = 0; j < shapePositions.Count; j++ ) {
                                        if( shapePositions[ j ] != Vector3.Zero ) { differentElements.Add( j ); }
                                    }
                                }
                            }
                        }
                            ...
                            for( var indexIdx = 0; indexIdx < indices.Count; indexIdx++ ) {
                                ...
                                if( differentElements.Contains( ( int )index ) ) {
                                    ShapeValueStructs.Add( new() {
                                        ...
                                        ReplacingVertexIndex =
                                            ( ushort )( differentElements.IndexOf( ( int )index ) + vertexCount +
                                                addedVertices ) // Later, will need to add this number to the total number of vertices (not including shapes)
                                    } );
                                }
                            }
                        ...
                        */
                        ...
                        if( !String.IsNullOrEmpty( material.Name ) && !materials.Contains( material.Name ) ) {
                            ...
                            if( materialIndex == -1 ) { materialIndex = materials.IndexOf( material.Name ); }
                        }
                    }
            ...
            if( skeleton != null ) { sortedBones = GetJoints( skeleton.GetJoint( 0 ).Joint.VisualChildren.ToList(), originalBoneIndexToString.Values.ToList() ); }
            ...
            foreach( var kvp in originalBoneIndexToString ) {
                ...
                if( index != -1 ) { originalBoneIndexToNewIndex.Add( kvp.Key, index ); }
            }
            ...
            for( var i = 0; i < SubmeshBoneMap.Length; i++ ) { SubmeshBoneMap[ i ] = ( ushort )i; }
            ...
            */
            ...
            foreach( var str in Strings ) { PluginLog.Debug( str ); }
            ...
            foreach( var meshIndex in _meshes.Keys ) {
                foreach( var submeshIndex in _meshes[ meshIndex ].Keys ) {
                    ...
                    for( var i = 0; i < submeshVertices.Count; i++ ) {
                        if( !vertexDict.ContainsKey( i ) ) { vertexDict.Add( i, new List< byte >() ); }
            ...
            foreach( var data in vertexDict.Values ) { vertexData.AddRange( data ); }
            ...
            for( var i = 0; i < BoneNameOffsets.Count; i++ ) { file.BoneBoundingBoxes[ i ] = zeroBoundingBoxStruct; }

            FillInValues( file );

            return ( file, vertexData, indexData );
        } catch( Exception ex ) { PluginLog.Debug( ex.ToString() ); } finally { PluginLog.Debug( "End" ); }
    ...
    private MdlStructs.LodStruct[] GetLodStruct( ushort meshCount ) {
        ...
        for( var i = 0; i < 3; i++ ) {
            ...
            if( i == 0 ) { ret[ i ].MeshCount = meshCount; }
        }
